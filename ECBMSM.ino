...
void processFinishedLine(String line) {
    ...
    if (colonIndex != -1) {
        ...
        // 1. อัปเดตค่าเกจวงกลม และข้อความตรงกลาง
        if(arc_soc) lv_arc_set_value(arc_soc, battery.stateOfCharge);
        if(label_soc_text) lv_label_set_text(label_soc_text, (String(battery.stateOfCharge) + "%").c_str()); 

        // 2. อัปเดตกล่องโวลต์และกระแส
        if(label_voltage) lv_label_set_text(label_voltage, (String(battery.totalVoltage, 2) + "V").c_str());
        if(label_current) lv_label_set_text(label_current, (String(battery.currentAmps, 1) + " A").c_str());

        // 3. อัปเดตแรงดันรายเซลล์ C1-C4 (แก้ไขบั๊กเพิ่มจุดสร้างข้อความเรียลไทม์)
        if(label_cells_footer) {
            String cells_str = "C1: " + String(battery.cell1, 3) + "V  C2: " + String(battery.cell2, 3) + "V\n" +
                               "C3: " + String(battery.cell3, 3) + "V  C4: " + String(battery.cell4, 3) + "V";
            lv_label_set_text(label_cells_footer, cells_str.c_str());
        }

        // 4. อัปเดตอุณหภูมิแยกชิ้นตามโครงสร้างจริงบนจอ
        if(label_temp_mos) lv_label_set_text(label_temp_mos, ("MOS: " + String(battery.mosfetTemp, 1) + "°C").c_str());
        if(label_temp_board) lv_label_set_text(label_temp_board, ("BAL: " + String(battery.boardTemp, 1) + "°C").c_str());
    }
}
...
void create_esmcomm_dashboard() {
    ...
    lv_label_set_text(label_title, "ECBMS-BL 2.0");
    ...
    lv_label_set_text(label_ip, "IP 192.168.10.42");
    ...
    lv_obj_set_style_text_font(label_panel_header, LV_FONT_DEFAULT, LV_PART_MAIN);
    lv_obj_align(label_panel_header, LV_ALIGN_TOP_LEFT, -5, -5);

    // ---- SoC Arc Gauge ----
    arc_soc = lv_arc_create(main_panel);
    lv_obj_set_size(arc_soc, 100, 100);
    ...
    label_soc_text = lv_label_create(main_panel); // แก้ไขบั๊ก Pointer ซ้อนเกจเพี้ยน
    lv_label_set_text(label_soc_text, "0%");
    lv_obj_set_style_text_color(label_soc_text, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(label_soc_text, LV_FONT_DEFAULT, LV_PART_MAIN);
    lv_obj_align_to(label_soc_text, arc_soc, LV_ALIGN_CENTER, 0, 0);

    // ---- Bus Voltage Box ----
    lv_obj_t * box_volt = lv_obj_create(main_panel);
    lv_obj_set_size(box_volt, 90, 40);
    lv_obj_align(box_volt, LV_ALIGN_TOP_LEFT, 105, 10);
    ...
    lv_obj_set_size(box_curr, 90, 40);
    ...
    lv_obj_align(lbl_cap_h, LV_ALIGN_TOP_LEFT, 105, 56);
    ...
    lv_obj_align(lbl_chg_h, LV_ALIGN_TOP_LEFT, 175, 56);

    lv_obj_t * lbl_dchg_h = lv_label_create(main_panel);
    lv_label_set_text(lbl_dchg_h, "DCH");
    ...
    lv_obj_align(label_capacity, LV_ALIGN_TOP_LEFT, 105, 72);
    ...
    lv_obj_align(label_chg_limit, LV_ALIGN_TOP_LEFT, 175, 72);
    ...
    lv_obj_set_style_text_color(lbl_sw3, lv_color_make(65, 105, 225), LV_PART_MAIN);
    lv_obj_set_style_text_font(lbl_sw3, &lv_font_montserrat_10, LV_PART_MAIN);
    lv_obj_align(lbl_sw3, LV_ALIGN_TOP_LEFT, 100, 96);

    sw_autodim = lv_switch_create(main_panel);
    lv_obj_set_size(sw_autodim, 32, 16);
    lv_obj_align(sw_autodim, LV_ALIGN_TOP_LEFT, 100, 110);
    lv_obj_add_state(sw_autodim, LV_STATE_CHECKED);

        // ---- TEMPERATURE BOX  ----
    lv_obj_t * box_temp = lv_obj_create(main_panel);
    lv_obj_set_size(box_temp, 150, 38);
    lv_obj_align(box_temp, LV_ALIGN_TOP_RIGHT, 5, 94);
    lv_obj_set_style_bg_color(box_temp, lv_color_make(24, 44, 70), LV_PART_MAIN);
    lv_obj_set_style_border_color(box_temp, lv_color_make(52, 73, 94), LV_PART_MAIN);
    lv_obj_set_style_border_width(box_temp, 1, LV_PART_MAIN);
    lv_obj_set_style_pad_all(box_temp, 0, LV_PART_MAIN);
    lv_obj_clear_flag(box_temp, lv_obj_flag_t(LV_OBJ_FLAG_SCROLLABLE));

    lv_obj_t * lbl_t_title = lv_label_create(box_temp);
    lv_label_set_text(lbl_t_title, "TEMPERATURE");
    lv_obj_set_style_text_color(lbl_t_title, lv_color_make(149, 165, 166), LV_PART_MAIN);
    lv_obj_set_style_text_font(lbl_t_title, &lv_font_montserrat_10, LV_PART_MAIN);
    lv_obj_align(lbl_t_title, LV_ALIGN_TOP_LEFT, 4, 2);
    
    label_temp_mos = lv_label_create(box_temp);
    lv_label_set_text(label_temp_mos, "MIN --°C");
    lv_obj_set_style_text_color(label_temp_mos, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(label_temp_mos, &lv_font_montserrat_10, LV_PART_MAIN);
    lv_obj_align(label_temp_mos, LV_ALIGN_BOTTOM_LEFT, 4, -2);

    label_temp_board = lv_label_create(box_temp);
    lv_label_set_text(label_temp_board, "MAX --°C");
    lv_obj_set_style_text_color(label_temp_board, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(label_temp_board, &lv_font_montserrat_10, LV_PART_MAIN);
    lv_obj_align(label_temp_board, LV_ALIGN_BOTTOM_RIGHT, -4, -2);

        // ---- Bottom Menu Bar ----
    lv_obj_t * tab_bar = lv_obj_create(scr);
    lv_obj_set_size(tab_bar, 320, 30);
    lv_obj_align(tab_bar, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(tab_bar, lv_color_make(10, 17, 26), LV_PART_MAIN);
    lv_obj_set_style_border_width(tab_bar, 0, LV_PART_MAIN);
    lv_obj_clear_flag(tab_bar, lv_obj_flag_t(LV_OBJ_FLAG_SCROLLABLE));

    lv_obj_t * btn_overview = lv_label_create(tab_bar);
    lv_label_set_text(btn_overview, "Overview");
    lv_obj_set_style_text_color(btn_overview, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(btn_overview, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_align(btn_overview, LV_ALIGN_LEFT_MID, 15, -2);

    lv_obj_t * line_indicator = lv_obj_create(tab_bar);
    lv_obj_set_size(line_indicator, 55, 2);
    lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_LEFT, 15, 0);
    lv_obj_set_style_bg_color(line_indicator, lv_color_make(52, 152, 219), LV_PART_MAIN);
    lv_obj_set_style_border_width(line_indicator, 0, LV_PART_MAIN);

    lv_obj_t * btn_summary = lv_label_create(tab_bar);
    lv_label_set_text(btn_summary, "Live Summary");
    lv_obj_set_style_text_color(btn_summary, lv_color_make(127, 140, 141), LV_PART_MAIN);
    lv_obj_set_style_text_font(btn_summary, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_align(btn_summary, LV_ALIGN_CENTER, 0, -2);

    lv_obj_t * btn_settings = lv_label_create(tab_bar);
    lv_label_set_text(btn_settings, "Settings");
    lv_obj_set_style_text_color(btn_settings, lv_color_make(127, 140, 141), LV_PART_MAIN);
    lv_obj_set_style_text_font(btn_settings, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_align(btn_settings, LV_ALIGN_RIGHT_MID, -25, -2);
}
    // ==========================================
...
void setup() {
    ...
    tft.begin();
    tft.setRotation(3); // บังคับแนวนอนสำหรับจอแสดงผลของบอร์ด CYD
