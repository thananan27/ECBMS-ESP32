/*
 * ECBMSM 2.1.1 Dashboard & BLE Gateway for ESP32-2432S028 (CYD 2.8" 320x240)
 * Touch Screen
...
void my_touchpad_read(lv_indev_t * indev, lv_indev_data_t * data) {
    ...
    // สั่งตรวจสอบสัญญาณนิ้วจิ้มสัมผัสหน้าจอ
    ...
    if (!touched) {
        ...
        // 🌟 จุดสำคัญ: ดักจับและป้องกันไม่ให้ค่าพิกัดทะลุติดลบ ( Underflow 65535 ) 🌟
        // ถ้าระบบอ่านค่าได้เกิน 4000 แปลว่าค่ามันติดลบ ให้ปัดสวิตช์แก้กลับเป็นเลข 0 ทันที
        int finalX = (touchX > 4000) ? 0 : touchX;
        int finalY = (touchY > 4000) ? 0 : touchY;

        // ล็อกไม่ให้พิกัดแกน X ทะลุเกินความกว้าง 320 พิกเซล
        if (finalX > 320) finalX = 320;
        // ล็อกไม่ให้พิกัดแกน Y ทะลุเกินความสูง 240 พิกเซล
        if (finalY > 240) finalY = 240;

        // ส่งข้อมูลพิกเซลที่เคลียร์ตัวเลขลบเสร็จแล้วไปให้ระบบปุ่มกดสั่งงาน
        data->point.x = finalX;
        data->point.y = finalY;

        // พิมพ์ตรวจเช็กพิกัดแกนพิกเซลตรงปุ่มหลังจากเคลียร์บั๊กแล้วทาง Serial Monitor
        //Serial.printf("พิกัดนิ้วตัวจริงบนปุ่ม -> X: %d, Y: %d\n", finalX, finalY);
    }
}
...
static void menu_click_cb(lv_event_t * e) {
    ...
    if (target == btn_overview) {
        // เมื่อจิ้มปุ่ม Overview -> สั่งซ่อนหน้า 2 และดึงหน้า 1 แสดงผล
        lv_obj_add_flag(cells_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(main_panel, LV_OBJ_FLAG_HIDDEN);
        
        // 🌟 ขยับเส้นใต้สีฟ้ามาจอดรองรับใต้คำว่า Overview ที่ตำแหน่งพิกัด X: 20 พอดี
        lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_LEFT, 20, 0);
    } 
    else if (target == btn_summary) {
        // เมื่อจิ้มปุ่ม Live Summary -> สั่งซ่อนหน้า 1 และดึงหน้า 2 แสดงผล
        lv_obj_add_flag(main_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(cells_panel, LV_OBJ_FLAG_HIDDEN);
        
        // 🌟 ขยับเส้นใต้สีฟ้าเลื่อนสไลด์ไปรอรับใต้คำว่า Live Summary ที่ตำแหน่งพิกัด X: 165 พอดี
        lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_LEFT, 165, 0);
    }
}
...
// ==========================================
    ...
    lcd.init();
    lcd.setRotation(1); // ล็อกแนวนอนสอดคล้องกับพอร์ต USB ด้านบน
    lcd.fillScreen(TFT_BLACK);

    // 🌟 นี่คือค่าคาลิเบรต 4 มุมของบอร์ดบอสจริง ๆ ครับ (ก๊อปปี้แถวนี้ไปใส่แทนตัวเดิมได้เลยครับบอส) 🌟
    uint16_t calData[8] = { 0, 0, 0, 237, 316, 0, 320, 237 };
    lcd.setTouchCalibrate(calData); // สั่งป้อนค่าขอบเขตตัวจริงเข้าสู่หัวใจชิปสัมผัส LGFX
    lv_init();
