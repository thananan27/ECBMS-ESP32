...
class LGFX_ESP32_2432S028 : public lgfx::LGFX_Device {
    ...
    lgfx::Touch_XPT2046  _touch_instance; // 🚨 1. เพิ่มตัวแปรสำหรับชิปสัมผัส XPT2046
public:
    LGFX_ESP32_2432S028() {
        {
        ...
        }
        {
            ...
            // 🚨 เพิ่มบรรทัดนี้เข้าไป เพื่อบังคับหมุนทิศทางพิกัดสัมผัสให้เท่ากับหน้าจอหลัก
...
lv_obj_t * btn_overview = nullptr;
lv_obj_t * btn_summary = nullptr;
...
#define DRAW_BUF_SIZE (SCREEN_WIDTH * 20 * 2) // จองไว้ 20 แถว แถวละ 2 ไบต์ต่อพิกเซล
static uint8_t lv_buf[DRAW_BUF_SIZE];
// ==========================================
...
// ฟังก์ชันสำหรับดักจับตำแหน่งที่นิ้วกดหน้าจอสัมผัส
void my_touchpad_read(lv_indev_t * indev, lv_indev_data_t * data) {
    ...
    if(!touched) {
        ...
        // 1. แปลงทิศทางแกนตามสูตรเดิมที่ทิศทางตรงแล้ว
        int rawX = 320 - touchY;
        int rawY = touchX;
        
        // 2. 🚨 ใช้ฟังก์ชัน map เพื่อขยายสเกลพิกัด 4 มุมของคุณ ให้ตรงกับหน้าจอระบบ (320x240) เป๊ะๆ
        int calX = map(rawX, 76, 297, 0, 320);
        int calY = map(rawY, 4, 306, 0, 240);
        
        // 3. ป้องกันไม่ให้ค่าพิกัดทะลุขอบจอ (จำกัดค่าให้อยู่ในช่วงหน้าจอพอดี)
        data->point.x = constrain(calX, 0, 319);
        data->point.y = constrain(calY, 0, 239);
        
        // บันทึก Log พิกัดใหม่ที่ผ่านการ Calibrate แล้ว
        Serial.printf("Final Matched Touch -> X: %d, Y: %d\n", data->point.x, data->point.y);
    }
}
...
static void menu_click_cb(lv_event_t * e) {
    ...
    if (target == btn_overview) {
        lv_label_set_text(label_title, "ESMCOMM 1.0");
        lv_obj_remove_flag(main_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(cells_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_LEFT, 23, 0); // ระยะพิกัดปุ่ม 1 ใหม่
    }
    else if (target == btn_summary) {
        lv_label_set_text(label_title, "PACK 01 DETAIL");
        lv_obj_add_flag(main_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(cells_panel, LV_OBJ_FLAG_HIDDEN);
        lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_MID, 0, 0);  // ระยะพิกัดปุ่ม 2 ใหม่
    }
}
...
void create_unified_dashboard() {
    ...
    // ==========================================
        // ---- วางแทนที่บรรทัดเดิมตรงนี้ ----
    lv_obj_t * content_body = lv_obj_create(scr);
    lv_obj_set_size(content_body, 320, 185); 
    lv_obj_align(content_body, LV_ALIGN_TOP_MID, 0, 25); 
    lv_obj_set_style_bg_opa(content_body, 0, LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(content_body, 0, LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(content_body, 0, LV_STATE_DEFAULT);
    lv_obj_clear_flag(content_body, LV_OBJ_FLAG_SCROLLABLE);

    main_panel = lv_obj_create(content_body); 
    // ---------------------------------
    ...
    // ==========================================
    cells_panel = lv_obj_create(content_body);
    ...
    lv_obj_set_size(tab_bar, 320, 35); // เพิ่มพื้นที่แถบเมนูให้กดง่ายขึ้น
    ...
    // 1. ปุ่มสลับหน้า 1 (Overview)
    btn_overview = lv_button_create(tab_bar); 
    lv_obj_set_size(btn_overview, 90, 30); 
    lv_obj_align(btn_overview, LV_ALIGN_LEFT_MID, 10, 0);
    lv_obj_set_style_bg_color(btn_overview, lv_color_make(10, 17, 26), LV_STATE_DEFAULT); 
    lv_obj_set_style_shadow_width(btn_overview, 0, LV_STATE_DEFAULT); 
    ...
    lv_obj_set_style_text_color(lbl_overview, lv_color_white(), LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(lbl_overview, &lv_font_montserrat_12, LV_STATE_DEFAULT);
    lv_obj_align(lbl_overview, LV_ALIGN_CENTER, 0, 0);
    // 🚨 เปิดสถานะให้ข้อความส่งผ่านแรงกดทะลุลงไปยังปุ่มเบื้องหลัง
    lv_obj_add_flag(lbl_overview, LV_OBJ_FLAG_EVENT_BUBBLE); 

    // 2. ปุ่มสลับหน้า 2 (Live Summary)
    btn_summary = lv_button_create(tab_bar);
    lv_obj_set_size(btn_summary, 110, 30); 
    lv_obj_align(btn_summary, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_bg_color(btn_summary, lv_color_make(10, 17, 26), LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_width(btn_summary, 0, LV_STATE_DEFAULT);
    lv_obj_add_event_cb(btn_summary, menu_click_cb, LV_EVENT_CLICKED, NULL);

    lv_obj_t * lbl_summary = lv_label_create(btn_summary);
    lv_label_set_text(lbl_summary, "Live Summary");
    lv_obj_set_style_text_color(lbl_summary, lv_color_make(127, 140, 141), LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(lbl_summary, &lv_font_montserrat_12, LV_STATE_DEFAULT);
    lv_obj_align(lbl_summary, LV_ALIGN_CENTER, 0, 0);
    // 🚨 เปิดสถานะให้ข้อความส่งผ่านแรงกดทะลุลงไปยังปุ่มเบื้องหลังเช่นกัน
    lv_obj_add_flag(lbl_summary, LV_OBJ_FLAG_EVENT_BUBBLE); 
    ...
    lv_obj_align(btn_settings, LV_ALIGN_RIGHT_MID, -15, 0);

    // เส้นขีดใต้บอกสถานะเมนู
    line_indicator = lv_obj_create(tab_bar);
    lv_obj_set_size(line_indicator, 65, 3);
    lv_obj_align(line_indicator, LV_ALIGN_BOTTOM_LEFT, 23, 0);
...
// ==========================================
    ...
    lcd.init();
    lcd.setRotation(1);
    // เพิ่มโค้ดบังคับคาลิเบรตหน้าจอก่อนเริ่มระบบ LVGL
if (lcd.touch()) {
  if (lcd.hasPalette()) {
    lcd.setColorDepth(8);
  }
  // แสดงข้อความบอกผู้ใช้บนหน้าจอ
  lcd.setTextSize(2);
  lcd.setTextColor(TFT_WHITE);
  lcd.setCursor(10, 10);
  lcd.print("Touch the 4 corners to calibrate");
  
  // คำสั่งเรียกหน้าจอคาลิเบรตมุมจุดสี่เหลี่ยม 4 มุมของ LovyanGFX
  uint16_t calibrateData[8];
  lcd.calibrateTouch(calibrateData, TFT_RED, TFT_WHITE, 30);
  ...
  lcd.fillScreen(TFT_BLACK);
    ...
    lv_display_set_flush_cb(disp, my_disp_flush);
     ...
     // 🚨 เปิดระบบลงทะเบียนหน้าจอสัมผัส (Touch Input Driver) สำหรับ LVGL v9
    lv_indev_t * indev = lv_indev_create();
    lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER); 
    lv_indev_set_read_cb(indev, my_touchpad_read); // ผูกฟังก์ชันอ่านค่าสัมผัสด้านบน
    ...
    //  แก้ไขโดยลบพารามิเตอร์ตัวหลังออก เพื่อเปิดช่องให้บลูทูธสตรีมข้อมูลได้อย่างต่อเนื่อง
    pNimBLEScan->setScanCallbacks(new AdvertisedDeviceCallbacks()); 
    pNimBLEScan->setInterval(100);
    pNimBLEScan->setWindow(100);
    pNimBLEScan->setActiveScan(true); 
    // เปลี่ยนจาก pNimBLEScan->start(0); เป็น:
    pNimBLEScan->start(10, false); // สแกน 10 วินาทีแล้วหยุดชั่วคราว

    bmsMutex = xSemaphoreCreateMutex();
}

    void loop() {
    // 1. อัปเดตระบบเวลาของ LVGL v9 โดยเปลี่ยนคำว่า settling_time เป็น millis()
    static uint32_t last_tick = millis();
    uint32_t current_time = millis();
    lv_tick_inc(current_time - last_tick);
    last_tick = current_time;

    // 2. เรียกตัวจัดการคิวประมวลผลกราฟิก
    lv_timer_handler();

    // 3. ระบบดักสัญญาณข้อมูลแพ็กเก็ตใหม่จาก BMS
    if (dataReadyToUpdate) {
        dataReadyToUpdate = false;
