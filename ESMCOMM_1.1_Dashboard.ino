...
void processFinishedLine(String line) {
    ...
    // เมื่อข้อความส่งมาสิ้นสุดชุดรอบ "jhstop" -> ทำการยิงอัปเดตข้อมูลขึ้นหน้าจอ UI ทันที
        ...
        lv_label_set_text(label_temp_mos, (String(battery.mosfetTemp, 1)  + "°C").c_str());
        lv_label_set_text(label_temp_board, (String(battery.boardTemp, 1) + "°C").c_str());
    }
}
...
void create_esmcomm_dashboard() {
    lv_obj_t * scr = lv_screen_active(); // LVGL v9 เปลี่ยนจาก lv_scr_act() เป็น lv_screen_active()
    ...
    lv_obj_set_style_text_color(lbl_sw3, lv_color_make(127, 140, 141), LV_PART_MAIN);
    lv_obj_set_style_text_font(lbl_sw3, &lv_font_montserrat_10, LV_PART_MAIN);
    lv_obj_align(lbl_sw3, LV_ALIGN_TOP_LEFT, 106, 96);

    sw_autodim = lv_switch_create(main_panel);
    lv_obj_set_size(sw_autodim, 32, 16);
    lv_obj_align(sw_autodim, LV_ALIGN_TOP_LEFT, 108, 110);
    ...
    lv_obj_set_style_pad_all(box_temp, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_clear_flag(box_temp, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_label_set_text(label_temp_board, "BAL 0°C");
    lv_obj_set_style_text_color(label_temp_board, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(label_temp_board, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_align(label_temp_board, LV_ALIGN_BOTTOM_LEFT, 4, -2);

    label_temp_mos = lv_label_create(box_temp);
    lv_label_set_text(label_temp_mos, "MOS 0°C");
    lv_obj_set_style_text_color(label_temp_mos, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_text_font(label_temp_mos, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_obj_align(label_temp_mos, LV_ALIGN_BOTTOM_RIGHT, -4, -2);
